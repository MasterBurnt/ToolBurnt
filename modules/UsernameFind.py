...
def __start__():
    try:
        def banner():
            clear()
            a = c5+f"""
_  _ ____ ____ ____ _  _ ____ _  _ ____  ____ _ _  _ ___  
|  | [__  |___ |__/ |\ | |__| |\/| |___  |___ | |\ | |  \ 
|__| ___] |___ |  \ | \| |  | |  | |___  |    | | \| |__/ {c1}Src\n"""
            for x in a:
                print(x,end = "")
                sys.stdout.flush()
                sleep(0.03)
...
{c3}{"*" * 61}
        ...
        #Internet connection
        try:
            requests.get('https://www.github.com/MasterBurnt', timeout = 2)
        except (requests. ConnectionError, requests. Timeout):
            print(c4+f'\n[!] {c1}Please check your internet connection!')
            exit(0)
            
        while 1:
            user = input(c2+f'[?] {c1}Please Enter The Target Username : {c7}').strip()
            if user == "":
                print(c4+f'\n[!] {c1}Please Enter A Valid Username! ')
                sleep(2)
                banner();banner1()
            else:
                break
        ...
        def go_site(url):
            session = get_session()   
            with session.get(url = url+user, timeout = 2) as response:
                if response.status_code == 200:
                    print(c2+f"[+] {c6}Find : {c1}{url}{user}")
...
\n{c1}R{c2}E{c3}V{c4}I{c5}E{c6}W\n{c3}p{c4}l{c5}e{c6}a{c7}s{c1}e {c2}w{c3}a{c4}i{c5}t{c6}{c7}{c1}. {c2}.{c3}.\n""")
        ...
        #OutPut
        if save == []:
            print(c4+f'\n[!] {c1}Nothing was found!')       
        else:
            print(c2+f"\n[s] {c1}Username Found On {len(save)} Sites!\n{c2}[s] {c1}The file was saved in a UsernameFind-History \n{c2}[s] {c1}called {user}.txt\n")
        ...
        if save == []:
            ...
            try:
                os.mkdir('UsernameFind-History')
            except:
                pass
                       
        os.chdir('UsernameFind-History')
